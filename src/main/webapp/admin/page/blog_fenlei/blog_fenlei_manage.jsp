...
function add(){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/houtai/blog/fenlei/add' //iframe的url
	});
}
...
function edit(id){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/houtai/blog/fenlei/edit?id='+id //iframe的url
	});
}
...
function del(ids){
	...
	$.post('/admin/blog/fenlei/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			reload_data();
...
function table_edit_update(id,field,value){
	$.post('/admin/blog/fenlei/update?'+field+'='+value,{id:id},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.closeAll();
...
function update_isUse(id,isUse){
	$.post('/admin/blog/fenlei/update',{id:id,isUse:isUse},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.msg('修改失败');
...
<div class="layui-table-toolbar" style="margin-bottom: 3px;">
	<div class="layui-btn-group">
 	 ...
 	 <div style="display: inline-block;">关于显示位置：【1=基础模块】 【】</div>
</div>
			...
			, element = layui.element; //元素操作
			  table.render({
			   ...
			   cols: [[
			      ...
			      ,{field:'pos', title: '显示位置', width:60, edit: 'text',style:'font-size: 12px;'  }
