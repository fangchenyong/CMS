...
<script>
$(function(){
	if("${down.downFenLeiId}"!=""){
		$("#downFenLeiId").val("${down.downFenLeiId}");
	}
	
	if("${down.display_mode}"!=""){
		$("#display_mode").val("${down.display_mode}");
	}
	...
	function save() {
		...
		var downFenLeiId = $("#downFenLeiId").val();
		...
		$.post(save_url, {
			title : app.title,
			content : app.content,display_mode:display_mode,
			contentNoTag : app.contentNoTag,downFenLeiId:downFenLeiId,
			summary : app.summary,
			isUse : isUse,
	...
	function uploadFile(file){
	    $.ajaxFileUpload({
	        url : '/admin/down/add_thumbnail_image', //用于文件上传的服务器端请求地址  
	        ...
	        success : function(result) //服务器成功响应处理函数  
	        {
	            ...
	            if(result.success){
	            	app.thumbnail_image = result.path;
	            	$("#thumbnail_image_div").empty();
	            	$("#thumbnail_image_div").append('<img style="width: 200px;" src="'+result.path+'" />');
	            }
	        },  
		...
		<div>
			<div class="layui-form layui-form-pane">
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" id="name" autocomplete="off"
							value="${down.title }" v-model="title" placeholder="请输入   标题"
							class="layui-input">
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<select name="downFenLeiId" id="downFenLeiId"
							lay-filter="downFenLeiId">
							<c:forEach var="downFenLei" items="${downFenLeiList}">
								<option value="${downFenLei.id}">${downFenLei.name}</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="layui-form-item">
					<label class="layui-form-label">搜索-关键字</label>
					<div class="layui-input-block">
						<input type="text" id="trueName" autocomplete="off"
							value="${down.keywords }" v-model="keywords"
							placeholder="手机评测，数码产品测评，科技产品测评" class="layui-input">
					</div>
				</div>

				<div class="layui-form-item">
					<label class="layui-form-label">搜索-描述</label>
					<div class="layui-input-block">
						<input type="text" id="trueName" autocomplete="off"
							value="${down.description }" v-model="description"
							placeholder="中国最具影响力的科技视频平台。创始人兼 CEO 王自如和 ZEALER 坚持打造兼具行业洞察力与品质感的科技视频内容，输出「科技  Plus」生活主张。目前涵盖消费电子、智能家居、出行方式、娱乐游戏等全方位的科技生活内容。"
							class="layui-input">
					</div>
				</div>
				...
				<div class="layui-form-item layui-form-text">
					...
					<div class="layui-input-block">
						<script id="editor" name="content" type="text/plain"
							style="width:100%;height:600px; z-index:1; "></script>
					</div>
				</div>
				...
				<div class="layui-form-item"
					...
					<div class="layui-input-block">
						...
						<div id="thumbnail_image_div" style="min-height: 10px;">
							<img style="width: 200px;" alt=""
								src="${down.thumbnail_image }" />
						</div>
					</div>
				</div>
...
<script type="text/javascript">
    ue.addListener("ready",function(){
    	// 通过ajax请求数据
    	UE.ajax.request("${pageContext.request.contextPath}/admin/down/findById",
    			{
    				method:"post",
    				async:false,
    				data:{"id":"${down.id}"},
