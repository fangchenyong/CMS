...
function reload_data(){
	...
	table.reload('table', {
		 where: {downFenLeiId:downFenLeiId,q:q},page: {
             curr: 1 //重新从第 1 页开始
         }
    });
}
...
//打开添加窗口
function add(){
	window.open("/houtai/down/add");
}

//打开编辑窗口
function edit(id){
	window.open('/houtai/down/edit?id='+id);
}
...
function del(ids){
	...
	$.post('/admin/down/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			reload_data();
...
function table_edit_update(id,field,value){
	$.post('/admin/down/update?'+field+'='+value,{id:id},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.closeAll();
...
function update_isUse(id,isUse){
	$.post('/admin/down/update',{id:id,isUse:isUse},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.msg('修改失败');
...
<script type="text/html" id="format_downFenLei">
{{#  if(d.downFenLeiId != null){ }}
	 {{ d.downFenLei.name }}
{{#  }  }}
</script>
			...
			, element = layui.element; //元素操作
			  table.render({
			   ...
			   cols: [[
			      ...
			      ,{field:'description', title: '搜索-描述', width:180, edit: 'text',style:'font-size: 12px;'}
			      
			      ,{field:'isUse', title: '是否显示', width:60, templet: '#format_isUse'  }
			      ,{field:'display_mode', title: '展示模式', width:70, templet: '#format_display_mode'  }
			      
			      ,{field:'downFenLeiId', title: '分类', width:120,templet: '#format_downFenLei'  }
