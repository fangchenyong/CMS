...
function reload_data(){
	...
	table.reload('table', {
		 where: {q:q,memoFenLeiId:memoFenLeiId},page: {
             curr: 1 //重新从第 1 页开始
         }
    });
}
...
function add(){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/houtai/memo/add' //iframe的url
	});
}
...
function edit(id){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/houtai/memo/edit?id='+id //iframe的url
	});
}
...
<div class="layui-form layui-form-pane" style="margin-bottom: 3px; "  >
	<div class="layui-form-item" style="margin-bottom: 1px; min-width:1051px;">
	    ...
	    <div class="layui-input-inline" style="width: 150px; margin-right: 0px;">
	      <select id="memoFenLeiId" lay-filter="memoFenLeiId">
...
<script type="text/html" id="format_memoFenLei">
{{#  if(d.memoFenLeiId != null){ }}
	 {{ d.memoFenLei.name }}
{{#  }  }}
</script>
...
<script>
		layui.use([ 'laydate', 'laypage', 'layer', 'table', 'carousel',
			...
			, element = layui.element; //元素操作
			  table.render({
			   ...
			   cols: [[
			      ...
			      ,{field:'memoFenLeiId', title: '分类', width:60,style:'font-size: 12px;',templet: '#format_memoFenLei'}
			...
			//监听工具条 table_bar
				...
				form.on('select(memoFenLeiId)', function(data){
					reload_data();
		        });
