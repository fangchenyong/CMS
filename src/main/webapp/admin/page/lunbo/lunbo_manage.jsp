...
function add(){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/houtai/lunbo/add' //iframe的url
	});
}
...
function edit(id){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/houtai/lunbo/edit?id='+id //iframe的url
	});
}
...
function del(ids){
	...
	$.post('/admin/lunbo/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			reload_data();
...
function table_edit_update(id,field,value){
	$.post('/admin/lunbo/update?'+field+'='+value,{id:id},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.closeAll();
...
function update_isUse(id,isUse){
	$.post('/admin/lunbo/update',{id:id,isUse:isUse},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.msg('修改失败');
...
<script type="text/html" id="format_image_url">
  {{#  if(d.image_url != null){ }}
<img style="width: 40px; height: 40px;" alt="" src="{{d.image_url}}" onclick="seeImg('{{d.image_url}}') " />
  {{#  } else { }}
  {{#  } }}
</script>
...
<script>
		layui.use([ 'laydate', 'laypage', 'layer', 'table', 'carousel',
			...
			, element = layui.element; //元素操作
			  table.render({
			    elem: '#table'
			    ,url: '/admin/lunbo/list',
			    height: 'full-100',
			   cols: [[
			      {checkbox: true, fixed: true}
			      ,{field:'image_url', title: '图片', width:80, templet: '#format_image_url',style:'font-size: 12px;'}
			      ,{field:'title', title: '标题', width:180, edit: 'text',style:'font-size: 12px;'}
			      ,{field:'url', title: '链接', width:180, edit:'text', style:'font-size: 12px;'}
			      ,{field:'orderNo', title: '位置', width:120,edit: 'text' }
			      
			      ,{field:'isUse', title: '是否显示', width:60, templet: '#format_isUse'  }
			      ,{field:'type', title: '跳转类型', width:120,templet: '#format_type'  }
