...
<title>${blogFenLei.name} - ${config.web_name}</title>
...
<div class="wrap">
	...
	<div class="model" style="background-color: #ece9e9;">
		<div class="content" style="padding:10px 0px 10px 0px; ">
					...
					<div class="m-tab-content" style="min-height: 500px;">
						<ul>
							<c:forEach var="blog" items="${blogList}" varStatus="status">
								<li><i></i><span><fmt:formatDate value="${blog.createDateTime}" pattern="yyyy-MM-dd HH:mm"/></span><a href="/a/blog/get?id=${blog.id}" title="${blog.title}">${blog.title}</a></li>
							</c:forEach>
						</ul>
					</div>
			...
			<div style="padding: 10px; border: 1px solid #dadada; border-radius: 5px; min-height: 500px; background-color: white; width: 270px; float: left;">
				<div>
					右侧，模块，设计中
				</div>
			</div>
